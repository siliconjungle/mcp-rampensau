/* rampensau.mcp.js – comprehensive RampenSau MCP wrapper
   ------------------------------------------------------------------ */
...
const norm01 = z
  .number()
  .refine((v) => v >= 0 && v <= 100, 'value must be between 0 and 100')
...
const RampBase = {
  ...
  hStart: z
    ...
    .min(0)
    .max(360)
    .describe('Starting hue in degrees.')
    .optional(),
  hStartCenter: norm01
  ...
  hCycles: z
    .number()
    .min(1)
    .describe('Number of hue wraps.')
    .default(1)
    .optional(),
  hueList: z
    .array(z.number().min(0).max(360))
    .describe('Explicit list of hues instead of algorithm.')
    .optional(),
  sRange: RangeTuple
...
const CurveExtra = {
  curveMethod: z
    .enum(['lamé', 'sine', 'power', 'linear'])
    .describe('Curve algorithm for HSL easing.')
    .optional(),
  curveAccent: z
    .number()
    .min(0)
    .max(5)
    .describe('Accent strength for curveMethod.')
    .optional(),
};

const FormatOpt = {
  format: z
    .enum(['array', 'css-hsl', 'css-oklch'])
...
export const rampensauSpec = {
  ...
  tools: [
    /* 1 · palette generation ------------------------------------- */
    {
      ...
      async execute(raw) {
        const { format = 'array', curveMethod, ...opts } = z
    ...
    /* 2 · unique random hues ------------------------------------- */
    {
      ...
      parameters: {
        ...
        total: z
          ...
          .min(1)
          .default(5)
          .describe('How many hues.')
          .optional(),
        minHueDiffAngle: z
          .number()
          .min(0)
          .max(360)
          .describe('Minimum separation between hues.')
          .optional(),
      },
      async execute(args) {
      ...
      parameters: {
        ...
        mode: z
          ...
          .transform((m) => {
            ...
            return m; // already canonical
          })
          .describe('CSS colour-space. Aliases like "css" accepted.'),
      },
